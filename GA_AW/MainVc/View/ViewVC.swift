...
class ViewVC{
     ...
     */
    func deviceInfo(mainView: UIView, device: String){
        mainView_ = mainView
    ...
    func createAllScrolls(){
        createUI.createScroll(baseView: mainView_, scroll: weather_Scroll, x: 20, y: mainInfo_View.frame.maxY + 10, width: mainW - 40, height: mainH - mainInfo_View.frame.height - 50, cornerRad: 0, backColor: .clear)
        createUI.createContentAreaScroll(scroll: weather_Scroll, contentSizeW: mainW - 40, contentSizeH: 2120, contentView: contentForScroll_View, color: backgroundViewsColor_)
        createUI.createScroll(baseView: hourForecast_View, scroll: hourForecast_Scroll, x: 0, y: 30, width: contentForScroll_View.frame.width, height: 120, cornerRad: 15, backColor: .yellow)
        createUI.createContentAreaScroll(scroll: hourForecast_Scroll, contentSizeW: 2000, contentSizeH: 120, contentView: contentForHourScroll_View, color: .clear)
    }
    ...
    func createAllTables(table: UITableView){
        createUI.createTable(table: table, baseView: forecastTable_View, x: 0, y: 30, width: weather_Scroll.frame.width, height: 600, cornerRad: 15, rowHeight: 60, separatorStyle: .singleLine, separatorColor: .white)
    }
    ...
    func createViewsForInfo(){
        let openMapButton = UIButton(),
            weatherInCityLabel = UILabel(),
            imageBottom = UIImageView()
        
        createUI.createButtons(userButton: openMapButton, baseView: contentForScroll_View, x: 0, y: pressure_View.frame.maxY + 10, width: contentForScroll_View.frame.width, height: 45, cornerRad: 0, color: .yellow)
        createUI.createLabels(userLabel: weatherInCityLabel, baseView: contentForScroll_View, color: .white, text: "Weather in city: Moscow", x: 0, y: openMapButton.frame.maxY + 10, width: contentForScroll_View.frame.width, height: 20, font: UIFont.boldSystemFont(ofSize: 16), textAlignment: .center)
        createUI.createImage(imageView: imageBottom, baseView: contentForScroll_View, x: contentForScroll_View.frame.width/2 - 12.5, y: weatherInCityLabel.frame.maxY + 10, width: 25, height: 25, color: .green, icon: imEye)
    }
}
